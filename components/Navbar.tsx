...
const LANG_LABELS: Record<Lang, string> = { en: 'EN', it: 'IT' }
const LANG_OTHER: Record<Lang, Lang>    = { en: 'it', it: 'en' }

export default function Navbar() {
  // Home: l'hero a fotogrammi è pinnato per 3× viewport → la nav resta visibile
  // durante l'animazione e si nasconde solo dopo, quando la pagina scorre davvero.
  const { scrolled, hidden }      = useHideOnScroll(3)
  ...
  const switchTo = (l: Lang) => { setLang(l); setDdOpen(false) }

  return (
    <nav className={`nav${scrolled ? ' scrolled' : ''}${hidden ? ' gone' : ''}`}>
      <a href={lp(lang, '/')} className="nav__logo" aria-label="Keecks">
      ...
      <div className="nav__actions">
        ...
        <div className="nav__lang" ref={ddRef}>
          <button className="nav__lang-btn" onClick={() => setDdOpen(v => !v)}>
            {LANG_LABELS[lang]}<span className="nav__lang-arrow" style={{ visibility: ddOpen ? 'hidden' : 'visible' }}> ▾</span>
          </button>
          {ddOpen && (
            <button className="nav__lang-btn nav__lang-other" onClick={() => switchTo(LANG_OTHER[lang])}>
              {LANG_LABELS[LANG_OTHER[lang]]}
            </button>
          )}
        </div>
      </div>
