...
export default function Navbar() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // Phase 2 hero ends at scrollY ≈ 3 × innerHeight (end: '+=300%')
      // Solo mobile: su desktop la nav resta sempre visibile
      if (window.innerWidth < 900) {
        setGone(window.scrollY >= window.innerHeight * 3)
      }
    }
  ...
  return (
    <nav className={`nav${scrolled ? ' scrolled' : ''}${gone ? ' gone' : ''}`}>
